...
// Interface for Privy wallet transaction signing
export interface PrivyWalletSigner {
  address: string;
  signTransaction: (params: { encodedTransaction: string }) => Promise<{ signature: string; encodedTransaction: string }>;
}
...
/**
 * Create token on pump.fun with Privy wallet integration (mint only, no initial buy)
 */
export async function createAndLaunchTokenWithPrivy(
  ...
  const creator = new PublicKey(privyWallet.address);
  let metadataUri = '';
  ...
  try {
    ...
    statusCallback({ 
      message: 'Building transaction with Privy wallet...', 
      step: 1, 
    ...
    statusCallback({ 
      message: 'Preparing to create token (mint only)...', 
      step: 1, 
      logs: [`Metadata URI: ${metadataUri}`, `Mint Address: ${mint.publicKey.toBase58()}`] 
    });

    // Only create the token and associated token account, no buy instruction
    const instructions = [createInstruction, createAtaInstruction];
    ...
    // Sign the mint keypair portion of the transaction
    transaction.sign([mint]);

    statusCallback({
      message: 'Signing transaction with Privy wallet...',
      step: 2,
      logs: [
        `Metadata URI: ${metadataUri}`, 
        `Mint Address: ${mint.publicKey.toBase58()}`,
        `Using Privy wallet: ${privyWallet.address}`
      ],
    });

    // Encode the transaction for Privy signing
    const serializedTransaction = Buffer.from(transaction.serialize()).toString('base64');
    
    // Sign with Privy wallet
    const privySignResult = await privyWallet.signTransaction({
      encodedTransaction: serializedTransaction
    });

    // Decode the signed transaction
    const signedTransactionBuffer = Buffer.from(privySignResult.encodedTransaction, 'base64');
    const signedTransaction = VersionedTransaction.deserialize(signedTransactionBuffer);
    ...
    statusCallback({
      ...
      logs: [
        ...
        `Mint Address: ${mint.publicKey.toBase58()}`, 
        `Signature: ${signature}`,
        `Signed with Privy wallet: ${privyWallet.address}`,
        `Note: Token created without initial buy`
      ],
    });
